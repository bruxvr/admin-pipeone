...
function updateMetrics() {
  document.getElementById('totalSubAccounts').textContent = formatNumber(appData.masterAccount.totalSubAccounts);
  document.getElementById('totalUsers').textContent = formatNumber(appData.masterAccount.totalUsers);
  document.getElementById('activeChannels').textContent = formatNumber(appData.masterAccount.activeChannels);
  document.getElementById('totalMessages').textContent = formatNumber(appData.masterAccount.totalMessages);
  document.getElementById('activeBulks').textContent = formatNumber(appData.masterAccount.activeBulks);
  document.getElementById('openConversations').textContent = formatNumber(appData.masterAccount.openConversations);
}
...
function applyFilters() {
  ...
  filteredData = appData.subAccounts.filter(account => {
    ...
    const matchesChannel = !channelFilter || 
      (account.channels[channelFilter] && account.channels[channelFilter].status === 'active');
...
function createTableRow(account) {
  ...
  const expandIcon = hasSubUnits ? 
    `<i class="fas fa-chevron-${expandedRows.has(account.id) ? 'down' : 'right'}"></i>` : '';
  
  row.innerHTML = `
    <td>
      <input type="checkbox" ${selectedSubAccounts.has(account.id) ? 'checked' : ''} 
             onchange="toggleSelection(${account.id})">
    </td>
    <td>
      <div style="display: flex; align-items: center; gap: 8px;">
        ${hasSubUnits ? `<button class="action-btn" onclick="toggleExpand(${account.id})" style="background: none; border: none; color: var(--color-text-secondary); cursor: pointer;">${expandIcon}</button>` : ''}
    ...
    <td>
      <div class="channels-container">
        ${Object.entries(account.channels).map(([channel, data]) => 
          `<div class="channel-icon ${channel} ${data.status}" title="${channel.charAt(0).toUpperCase() + channel.slice(1)}: ${statusTranslations[data.status]}">
    ...
    <td>
      <div class="activity-metrics">
        ...
        <div class="activity-item">
          ...
          <span>${account.activity.dailyMessages.sent + account.activity.dailyMessages.received} msgs</span>
        </div>
      </div>
    </td>
    ...
    <td>
      <div class="action-buttons">
        <button class="action-btn edit" onclick="editSubAccount(${account.id})" title="Editar">
          <i class="fas fa-edit"></i>
        </button>
        <button class="action-btn toggle" onclick="toggleSubAccount(${account.id})" title="Ativar/Desativar">
          <i class="fas fa-power-off"></i>
        </button>
        <button class="action-btn report" onclick="generateReport(${account.id})" title="Relatório">
...
function createSubUnitRow(unit) {
  ...
  row.innerHTML = `
    ...
    <td>
      <div class="action-buttons">
        <button class="action-btn edit" onclick="editSubUnit('${unit.id}')" title="Editar">
          <i class="fas fa-edit"></i>
        </button>
      </div>
    </td>
...
function createSubAccountCard(account) {
  ...
  const activeChannels = Object.entries(account.channels).filter(([_, data]) => data.status === 'active').length;
  const totalMessages = Object.values(account.channels).reduce((sum, data) => sum + data.messages, 0);
  
  card.innerHTML = `
    ${account.internalUnits.length > 0 ? `<div class="sub-units-indicator">${account.internalUnits.length} unidades</div>` : ''}
    ...
    <div class="card-actions">
      <button class="btn btn--sm btn--primary" onclick="editSubAccount(${account.id})">
        <i class="fas fa-edit"></i>
        Editar
      </button>
      <button class="btn btn--sm btn--outline" onclick="toggleSubAccount(${account.id})">
        <i class="fas fa-power-off"></i>
        ${account.status === 'active' ? 'Desativar' : 'Ativar'}
      </button>
      <button class="btn btn--sm btn--outline" onclick="generateReport(${account.id})">
        <i class="fas fa-chart-bar"></i>
        Relatório
      </button>
    </div>
...
function createTreeItem(account) {
  ...
  item.innerHTML = `
    <div class="tree-item-header ${isExpanded ? 'expanded' : ''}" onclick="toggleTreeItem(${account.id})">
      <div class="tree-expand-icon ${isExpanded ? 'expanded' : ''}" style="cursor: pointer;">
        ${hasSubUnits ? '<i class="fas fa-chevron-right"></i>' : ''}
      </div>
      <div class="tree-item-content">
        <div class="tree-item-info">
          <div class="tree-item-name">${account.name}</div>
          <div class="tree-item-details">${account.companyId} • ${formatNumber(account.userCount)} usuários</div>
        </div>
        <div class="tree-item-metrics">
          ...
          <div class="channels-container">
            ${Object.entries(account.channels).filter(([_, data]) => data.status === 'active').map(([channel, _]) => 
              `<div class="channel-icon ${channel} active" title="${channel.charAt(0).toUpperCase() + channel.slice(1)}">
                <i class="${channelIcons[channel]}"></i>
              </div>`
            ).join('')}
          </div>
          <div class="action-buttons">
            <button class="action-btn edit" onclick="event.stopPropagation(); editSubAccount(${account.id})" title="Editar">
              <i class="fas fa-edit"></i>
            </button>
            <button class="action-btn report" onclick="event.stopPropagation(); generateReport(${account.id})" title="Relatório">
              <i class="fas fa-chart-bar"></i>
            </button>
          </div>
        </div>
    ...
    ${hasSubUnits ? `
      <div class="tree-sub-items ${isExpanded ? 'expanded' : ''}">
        ${account.internalUnits.map(unit => `
          <div class="tree-sub-item">
            <div class="tree-sub-item-name">${unit.name}</div>
            <div class="tree-sub-item-details">${formatNumber(unit.userCount)} usuários • ${statusTranslations[unit.status]}</div>
          </div>
        `).join('')}
      </div>
    ` : ''}
...
function updateSelectAllCheckbox() {
  ...
  if (selectAll) {
    ...
    const selectedVisibleCount = visibleIds.filter(id => selectedSubAccounts.has(id)).length;
  ...
  showModal('Editar Subconta', `
    ...
    <div class="form-group">
      ...
      <select class="form-control">
        ...
        <option value="inactive" ${account.status === 'inactive' ? 'selected' : ''}>Inativo</option>
        <option value="warning" ${account.status === 'warning' ? 'selected' : ''}>Atenção</option>
      </select>
    </div>
    <div class="form-group">
...
function editSubUnit(id) {
  showModal('Editar Unidade Interna', `
    <div class="form-group">
...
function toggleSubAccount(id) {
  ...
  showModal('Status Alterado', `
    <p>A subconta "${account.name}" foi ${newStatus === 'active' ? 'ativada' : 'desativada'} com sucesso.</p>
